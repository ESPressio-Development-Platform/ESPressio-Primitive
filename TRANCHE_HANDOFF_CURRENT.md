...
Date: 2026-09-14
Continuation state: structural Tranches 2–9 CLOSED; Tranche 10 ACTIVE at D10-10
...
10. D10-10 Lua Event operations through final Event APIs — **ACTIVE NEXT**
...
## Immediate continuation — D10-10

D10-10 adds Lua Event operations through the final Event family APIs.

Before writes:

1. re-query exact Lua and Event `primitives_redesign` tips and inspect both manifest boundaries;
2. consume the existing immutable `EventTypeDescriptor`, frozen TypeDirectory and family-owned `Event::DispatchDynamicEvent`; do not add a Lua Event registry or family-extension authorization handle;
3. require explicit application-owned authorization before dynamic construction/dispatch;
4. bound DirectBinary/CBOR/JSON P3 input using final Serializable/Event metadata before family decode/construction;
5. preserve Event-family admission/dispatch status and do not create listener registration, routing, retry, transport, worker or occurrence-ID ownership in Lua;
6. validate against a real final Event runtime whose listener/target topology is bound before `Runtime::Start()`, with canonical provider-generated P3 bytes, invalid schema, denied authorization, nonconstructible/local-type and capacity/bounds evidence;
7. run exact-tip focused and aggregate Lua workflows, update this handoff, then continue directly to D10-11.
